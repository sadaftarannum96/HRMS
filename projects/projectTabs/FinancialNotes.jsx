...
const FinancialNotes = ({projectId}) => {
  const {permissions} = useContext(AuthContext);
  const [isOptionsPopoverOpen, setIsOptionsPopoverOpen] = useState(null);
  ...
  const fetchNotes = async () => {
    setIsLoading(true);
    const [err, res] = await until(getNotes(projectId, 'FinancialNotes'));
    setIsLoading(false);
    if (err) {
